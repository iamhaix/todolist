<?php
   ...
   abstract class todolistDb
    {
        ...
        public static function addTask( $taskinfo){
				...
				$str1 = strlen($taskinfo['title']);
				$str2 = strlen($taskinfo['content']);
                ...
                $condition = 'androiddbid = '.$taskinfo['androiddbid'];
                $table = 'tasktable';
	   ...
	   public static function setMaxUSN( $uid )
      {
       ...
       public static function getBatchTask( $uid )
        {       
                ...
                $data = $myDB->getRows('SELECT  * FROM ' . self::TASKTABLE . ' WHERE USERID = ' .$uid . ' AND DATE = date_format(curdate(),"%Y-%m-%d") AND STATE = 0');
                if( false === $data )
                {
                        return false;
                        //错误处理
				...
				$data1 = $myDB->getRows('SELECT  * FROM ' . self::TASKTABLE . ' WHERE USERID = ' .$uid . ' AND DATE = date_format(curdate(),"%Y-%m-%d") AND STATE = 1');
                if( false === $data1 )
                {
                        return false;
                        //错误处理
                ...
                $data2 = $myDB->getRows('SELECT  * FROM ' . self::TASKTABLE . ' WHERE USERID= ' . $uid .' AND DATE > date_format(curdate(),"%Y-%m-%d")');
                if( false === $data2 )    
                {
                        return false;
                        //错误处理
  ...
  function synchTask($param)
  {
	 ...
	 FullSync($param);
	 if( $lastupdatecount < $mylastupdatecount )
	 {   sendChanges($Param);
	 }
	 else{
	    $response['code'] = 11;
        $response['message'] = '获取已完成任务成功失败';
		echo json_encode($response);
	 }
  }
  ...
  function FullSync($taskinfo)
  {
	...
	// print_r($android_datas);
	 ...
	 {  
	   ...
	   // echo "<<<serverdbid".$serverdbid.">>>>>serverdbid";
       // echo "<<<id".$single_data[id]."id>>>>";
		...
		{ // echo "<<<serverdbid".$serverdbid.">>>>>serverdbid";
	      // echo "<<<id".$single_data[id]."id>>>>";
		   ...
		   /*usn属性为空，事件从没有上传到数据库，说明任务是android端新增加的，需上传其到server数据库中*/
		   else
		   {     
				...
				/*  $condition = 'androiddbid = '.$androiddbid;
				  $table = 'tasktable';
				  $myDB = getInstance();
				  $result = $myDB->getRowsCount($table,$condition);
				  if($result)
  ...
  /*同步函数，如果两端的版本号相同，执行sendChanges函数*/
  function sendChanges( $taskinfo )
  {    
	 ...
	 $add_to_android = $data1;
      ...
      foreach ($android_datas as $single_data)
	  {
		...
		{  
		   ...
		   if( $Param['dirty'] == 2 || $Param['dirty'] == 3)
		   {    
		        $Param = $single_data;
