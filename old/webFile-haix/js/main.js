...
function getafterclock() {
	...
	jq.post("../Event.DB.php", para, function(data) {
		...
		if (afterclock.length > 0) {
			for (var i = 0; i < afterclock.length; i++) { (function(index) {
					var setClock = "clock" + afterclock[index].id;
					var startDate = new Date();
					var endDate = new Date(afterclock[index].remindtimestamp);
					var duration = endDate.getTime() - startDate.getTime();
					var clearId = afterclock[index].id;
					var title = afterclock[index].title;
					setClock = function() {
						jq(".loginDiv").hide();
						jq(".registerDiv").hide();
						jq(".remindDiv").hide();
	...
	jq.post("../Event.DB.php", para, function(rt) {
		...
		if (rt.code == 1) {
			for (var i = 0; i < rt['data']['todaydata'].length; i++) {
				...
				title = rt['data']['todaydata'][i]['title'].replace(/\'/g,'&#39;');
				...
				jq("#editComment" + taskid).css("display", "none");
			...
			for (var i = 0; i < rt['data']['todayfinishdata'].length; i++) {
				...
				title = rt['data']['todayfinishdata'][i]['title'].replace(/\'/g,'&#39');
			...
			for (var i = 0; i < rt['data']['futuredata'].length; i++) {
				...
				var title = rt['data']['futuredata'][i]['title'].replace(/\'/g,'&#39;');
			...
			for (var i = 0; i < rt['data']['futurefinishdata'].length; i++) {
				...
				var title = rt['data']['futurefinishdata'][i]['title'].replace(/\'/g,'&#39;');
		...
		if (!getCookie("uid")) {
			...
			if (title.trim(" ") == "") {
				...
				if(time!=""&&(new Date(tmp+" "+time))<=date){
					document.getElementById("textError").innerHTML = "闹钟时间过期,请重新选择";
					}
				else{	
	...
	jq("#confirmCreateTaskFuture").click(function() {
		if (!getCookie("uid")) {
			...
			if (title.replace(" ", "") == "") {
				...
				tmp = date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate();
			    if(dateFuture==""){
					    dateFuture=date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + (date.getDate()+1);
				}
				...
				//	console.log("data:" + data);
					...
					if (rt.code == 1) { //console.log(title);
						...
						jq("#" + rt.taskid).append(" <div class='completeDate' id='completeDate" + rt.taskid + "'></div>").append("<h3 id='clockDisplay" + rt.taskid + "' style='visibility:hidden;'>00:00</h3>").append("<div class='inline'><img id='clockIcon" + rt.taskid + "' src='images/icon07.png'></div>").append("<dl><dt><input class='eventClass  eventClass" + rt.taskid + "' id='event" + rt.taskid + "' type='text' value='" + title + "' readonly='readonly' size='1' maxlength='20'/></dt></dl>").append("<dl class='dealTask_right' id='editTask" + rt.taskid + "'><img id='edit" + rt.taskid + "' class='edit' title='编辑任务' src='images/icon03.png'> <img id='clockSet" + rt.taskid + "' class='clockSet' title='添加提醒' src='images/icon02.png'><input type='text' id='timepicker"+rt.taskid + "' onclick='WdatePicker({dateFmt:\"HH:mm\"})' class='WdatePicker'/><img id='deleteTask" + rt.taskid + "' class='deleteTask' title='删除' src='images/icon04.png'><button id='confirmTask" + rt.taskid + "' class='confirmTask' type='button'>确定</button><button id='cancelTask" + rt.taskid + "' class='cancelTask' type='button'>取消</button><button id='confirmClock" + rt.taskid + "' class='confirmClock' type='button'>确定</button><button id='cancelClock" + rt.taskid + "' class='cancelClock' type='button'>取消</button></dl>").append("	<div class='dealTask_left'><span id='updateComment" + rt.taskid + "' style='display:none;margin-left: 325px;'> <textarea class='commentClass' id='commentClass" + rt.taskid + "' name='textarea' rows='10' cols='20' style='margin-bottom:-10px;overflow-y:hidden;height:80px;'></textarea><button id='confirmComment" + rt.taskid + "' class='confirmComment'  type='button'>添加</button><button id='cancelComment" + rt.taskid + "' class='cancelComment' type='button'>取消</button>	</span>	<pre id='detailComment" + rt.taskid + "'  class='detailComment'></pre><a href='javascript:void(0)' id='editComment" + rt.taskid + "' class='editComment' style='margin-left:5px;'>编辑备注</a></div>");
...
/*			jq("#event" + id).attr("readonly", "readonly");
	...
	jq(".confirmComment").live('click', function() {
		if (!getCookie("uid")) {
			...
			jq('#detailComment' + id).text(jq("#commentClass" + id).attr('value'));
	...
	jq(".clockSet").live('click', function() {
		...
		var options = "<select id='hourselect" + id + "' name='hourselect'>";
		for (var i = 1; i < 24; i++) {
			if (i == hour) {
		...
		options += '</select>点<select id="minuteselect' + id + '" name="minuteselect">';
