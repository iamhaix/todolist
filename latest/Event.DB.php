<?php
   ...
   function modifyTask( $Param ){
      ...
      $taskId = $taskInfo['id'];
	  unset( $taskInfo['id'] );
	  $androiddbid = $taskInfo['androiddbid'];
	  unset( $taskInfo['androiddbid']);
 ...
 /* function  addAndroidId($param)
  ...
  function FullSync($taskinfo)
  {
	  ...
	  if(empty($mdatas))
	  {
		  $data1 = $myDB->getRows('SELECT  * FROM   tasktable  WHERE USERID = '.$uid.' AND STATE != 2');
	  }
	  else
	  {
		  foreach ($mdatas as $sdata)
			  $exitID[$indx++] = $sdata['serverdbid'];

		  $exitID = implode(",",$exitID);
		 // $sql = "SELECT  * FROM   tasktable  WHERE USERID = ".$uid." AND STATE != 2 and id not in ($exitID)";
		  // echo $sql;
		  $data1  = $myDB->getRows("SELECT  * FROM   tasktable  WHERE USERID = ".$uid." AND STATE != 2 and id not in ($exitID)");
	  }
	 ...
	 {  
		...
		{  
		  ...
		  {  
			  ...
			  $res[0]['androiddbid']= $single_data['id'];
		  ...
		  {   
			  ...
			  //echo "<<sin".$single_data['edittimestamp'].">>";
			  //echo "<<my".$mydata['edittimestamp'].">>";

			  if($single_data['edittimestamp'] > $mydata['edittimestamp'])
   			  { 
			         $Param = $single_data;
				     unset($Param ['dirty']);
				     unset($Param ['serverdbid']);
			  ...
			  else if($single_data['edittimestamp'] < $mydata['edittimestamp'])
			  {
				 // print_r($mydata);
				  $mydata['androiddbid'] = $single_data['id'];
				  $update_to_android[$index2++] = $mydata;
			  }
		   }
